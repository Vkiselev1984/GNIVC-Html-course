[**ВВЕДЕНИЕ В HTML**](#введение-в-html)

[**СОЗДАЕМ НОВЫЙ ПРОЕКТ**](#создаем-новый-проект)

> [**САМОСТОЯТЕЛЬНАЯ РАБОТА № 1**](#самостоятельная-работа-1)

[**СТРОЧНЫЕ И БЛОЧНЫЕ ЭЛЕМЕНТЫ В HTML**](#строчные-и-блочные-элементы-в-html)

> [СТРОЧНЫЕ ЭЛЕМЕНТЫ](#строчные-элементы)
>
> [БЛОЧНЫЕ ЭЛЕМЕНТЫ](#блочные-элементы)
>
> [**САМОСТОЯТЕЛЬНАЯ РАБОТА № 2**](#самостоятельная-работа-2)

[**КОНТЕНТ**](#контент)

> [ПОПУЛЯРНЫЕ ТЕГИ](#популярные-теги)
>
> [НЕСТАНДАРТНЫЕ ТЕГИ](#нестандартные-теги)

[**СОДЕРЖАНИЕ СТРАНИЦЫ**](#содержание-страницы)

> [О ФНС РОССИИ](#о-фнс-россии)
>
> [**САМОСТОЯТЕЛЬНАЯ РАБОТА № 3**](#самостоятельная-работа-3)

[**ОФОРМЛЕНИЕ СТРАНИЦЫ С ПОМОЩЬЮ CSS**](#оформление-страницы-с-помощью-css)

> [FLEX - КОНТЕЙНЕР](#flex---контейнер)
>
> [**САМОСТОЯТЕЛЬНАЯ РАБОТА № 4**](#самостоятельная-работа-4)
>
> [GRID - КОНТЕЙНЕР](#grid---контейнер)
>
> [БАННЕР С ЗАГОЛОВКОМ](#баннер-с-заголовком)
>
> [КОЛОНКА С СОДЕРЖАНИЕМ](#колонка-с-содержанием)
>
> [**САМОСТОЯТЕЛЬНАЯ РАБОТА № 5**](#самостоятельная-работа-5)

[**ЭЛЕМЕНТЫ ВЕРСТКИ САЙТА ФНС РОССИИ**](#элементы-верстки-сайта-фнс-россии)

> [СОДЕРЖАНИЕ](#содержание)
>
> [ОБЫЧНЫЕ ЗАГОЛОВКИ](#обычные-заголовки)
>
> [ПАРАГРАФ](#параграф)
>
> [ЭЛЕМЕНТ ФАЙЛ](#элемент-файл)
>
> [ЭЛЕМЕНТ СПИСОК](#элемент-список)
>
> [БЛОК ВАЖНО](#блок-важно)
>
> [БЛОК ВАЖНО НА ГОЛУБОМ ФОНЕ](#блок-важно-на-голубом-фоне)
>
> [ПОКАЗАТЬ ПОДРОБНОСТИ](#показать-подробности)
>
> [ОРАНЖЕВЫЙ БУЛЛИТ](#оранжевый-буллит)
>
> [ССЫЛКА НА СЕРВИС](#ссылка-на-сервис)
>
> [ПЕРЕМЕННЫЕ](#переменные)
>
> [ДВА СТОЛБЦА](#два-столбца)
>
> [БОЛЬШАЯ ЦИФРА НА СИНЕМ ФОНЕ](#большая-цифра-на-синем-фоне)
>
> [ВКЛАДКИ](#вкладки)
>
> [**САМОСТОЯТЕЛЬНАЯ РАБОТА № 6**](#самостоятельная-работа-6)
...
Если взглянуть на абстрактный блок (любой из представленных в прототипе) то его структура при более высокой детализации будет выглядеть следующим образом:

![Блочная модель в html](img/image9.png)

Область контента **CONTENT** - это область, в которой хранится содержимое нашего блока.

Далее на схеме изображен внутренний отступ **PADDING** от границы блока **BORDER** и внешний отступ **MARGIN**.
...
Данный пример демонстрирует поведение текстового содержания и изображений при использовании внутренних отступов. В первом случае текст имеет небольшой отступ, а в случае с изображением такого отступа нет.

Внешние отступы (margin) позволили карточкам задать расстояние друг от друга.
...
Начнем со структуры нашего проекта, которая будет состоять из **index.html** страницы и папок:
...
Во вкладку ![](img/image29.png) добавляют html код.

Во вкладку ![](img/image51.png) вносят CSS свойства элементов html.

Для сдачи контрольной работы необходимо добавить название проекта, сохранить его, нажав кнопку ![](img/image56.png) в правом верхнем углу. 

Копируем ссылку и отправляем преподавателю на проверку.

![](img/image45.png)

Изображения в html можно добавлять с помощью элемента "Изображение": **\<img src="ссылка"\>**

Для тестирования страницы будет использоваться панель разработчика браузера, которую можно открыть в браузере нажатием сочетания клавиш ![](img/image25.png), либо кликнуть правой кнопкой мышки на интересующем элементе веб-страницы, после чего выбрав пункт выпадающего списка "Посмотреть код":
...
Пример решения можно посмотреть и скачать в [репозитории](https://github.com/Vkiselev1984/GNIVC-Html-course) GitHub или к материалам урока. В примере вы найдете логотип ФНС России в
...
Не забываем про панель разработчика, в которой мы будем отслеживать поведение элементов на странице. При наведении на элемент в левой колонке мы увидим ширину элемента и его отступы относительно
...
Строчные элементы -- это элементы, ширина которых по горизонтали определяется шириной их содержимого. Например, теги **<img>** и **<a href="">ссылка</a>** - строчные. Таким элементам нельзя задать высоту и ширину.

На скриншоте видно, что блок img занял ширину в 60px, которую мы задали в html, и пропорциональную этому значению высоту. Текст же занимает ширину содержимого. Так как оба элемента строчные, то они будут располагаться в одной строке, следуя друг за другом.

В этом случае можно воспользоваться специальным элементом переноса на новую строку - тег **<br>**

Обратите внимание, что данный элемент состоит только из открывающего тега.

В Html существуют парные и одинарные теги. Последних не так много.

Примеры таких тегов: **<br>**, **<hr>**, **<img>**. В остальных случая необходимо обязательно удостоверится, что вы закрыли парный тег, иначе его свойства будут распространяться на последующие элементы страницы.
